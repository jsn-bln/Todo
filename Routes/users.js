...
router.put('/register', (req, res) => {
    Users.findOne({username : req.body.username}, async (err, match) => {
        ...
        if(match) return res.status(400).json({message: `username ${req.body.username} already exist`})
        if(!match){
...
router.get('/login', (req, res) => {
    Users.findOne({ username : req.body.username}, async (err, account) => {
        if(err) return res.status(500).json({message: err.message})
        if(!account) return res.status(400).json({message: ` username ${req.body.username} is not found!`})
        if(account){
